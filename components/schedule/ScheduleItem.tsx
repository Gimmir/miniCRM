...
export const ScheduleItem = ({
  ...
  return (
    <div
      ...
      className={`relative flex flex-col bg-white rounded-2xl mb-3 last:mb-0 transition-all duration-300 cursor-pointer overflow-hidden select-none border border-slate-100 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.03)] ${
        isCancelled ? 'bg-slate-50/60' : 'active:bg-slate-50'
      } ${isExpanded ? 'ring-2 ring-blue-500/10 shadow-md' : ''}`}
    >
      <div
        className={`absolute left-0 top-0 bottom-0 w-1 ${
          isCancelled ? 'bg-red-400' : 'bg-emerald-500'
        }`}
      ></div>
      ...
      <div
        className={`bg-slate-50/50 border-t border-slate-100 px-3 transition-all duration-300 ease-in-out overflow-hidden ${
      ...
      >
        <div className="flex gap-2">
          ...
          <button
            onClick={(e) => handleAction(e, onReschedule)}
          ...
          <button
            onClick={(e) => handleAction(e, onDetails)}
