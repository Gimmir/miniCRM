        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-3 px-1 relative">
            ...
            >
              ...
              <span>{sortOrder === 'alphabetical' ? 'А-Я' : sortOrder === 'recent' ? 'Недавні' : 'Давні'}</span>
            </button>
            ...
            {isSortMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-8 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-1 z-50 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                  <button onClick={() => selectSort('alphabetical')} className={`w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 ${sortOrder === 'alphabetical' ? 'text-blue-600 font-bold bg-blue-50/50' : 'text-slate-600'}`}>За алфавітом</button>
                  <button onClick={() => selectSort('recent')} className={`w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 ${sortOrder === 'recent' ? 'text-blue-600 font-bold bg-blue-50/50' : 'text-slate-600'}`}>Спочатку недавні</button>
                  <button onClick={() => selectSort('oldest')} className={`w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 ${sortOrder === 'oldest' ? 'text-blue-600 font-bold bg-blue-50/50' : 'text-slate-600'}`}>Спочатку давні</button>
                </div>
              </>
            )}
          </div>
